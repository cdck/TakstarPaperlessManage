package com.xlk.takstarpaperlessmanage.ui.gl;
...
public class MyGLSurfaceView extends GLSurfaceView {
    ...
    public void setCodecType(int codecType) {
        if (glRender == null) {
            LogUtils.i(TAG, "setCodecType -->glRender为null");
            initRender();
        }
    ...
    public void initCodec(String mimeType, int w, int h, byte[] codecdata) {
        if (!saveMimeType.equals(mimeType) || initW != w || initH != h || mediaCodec == null) {
            ...
            try {
                ...
                LogUtils.i(TAG, "initCodec -->configure surface是否为空: " + (surface == null));
    ...
    private void initMediaFormat(int w, int h, byte[] codecdata) {
        LogUtils.d(TAG, "initMediaFormat :   -->mediaFormat是否为空： " + (mediaFormat == null) + ", w= " + w + ", h= " + h);
